...
export function CrmPanel({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-[32px] border border-white/10 bg-[linear-gradient(180deg,rgba(17,17,19,0.98),rgba(11,11,13,0.98))] shadow-[0_16px_60px_rgba(0,0,0,0.28)] ${className}`}
    >
      {children}
    </div>
  );
  ...
  return (
    <div className="mb-7 flex flex-wrap items-start justify-between gap-4">
      <div className="max-w-3xl">
        ...
        {description ? <div className="mt-2 max-w-2xl text-sm leading-6 text-zinc-500">{description}</div> : null}
      </div>
...
export function CrmMetricCard({
  ...
  return (
    <div className="relative flex h-full min-h-[168px] flex-col overflow-hidden rounded-[28px] border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(59,255,140,0.1),transparent_36%),linear-gradient(180deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))] p-5">
      ...
      <div className="mt-auto pt-3 text-sm leading-6 text-zinc-400">{helper}</div>
    </div>
  );
}
...
export function CrmInsightCard({
  ...
  return (
    <div className="h-full rounded-[24px] border border-white/10 bg-black/20 p-4">
      <div className="text-[11px] uppercase tracking-[0.2em] text-zinc-500">{label}</div>
...
export function CrmQuickActionButton({
  ...
  return (
    <button
      ...
      className={
        emphasis === 'primary'
          ? 'w-full rounded-[22px] border border-[#3BFF8C]/20 bg-[radial-gradient(circle_at_left,rgba(59,255,140,0.14),transparent_70%),rgba(59,255,140,0.08)] px-4 py-3 text-left text-sm font-medium text-white transition hover:border-[#3BFF8C]/35 hover:bg-[radial-gradient(circle_at_left,rgba(59,255,140,0.18),transparent_72%),rgba(59,255,140,0.12)]'
          : 'w-full rounded-[22px] border border-white/10 bg-white/5 px-4 py-3 text-left text-sm text-white transition hover:bg-white/10'
      }
    >
      {label}
    </button>
  ...
  return (
    <div>
      ...
      <div className="h-2 rounded-full bg-white/8">
        <div
          className="h-2 rounded-full bg-[linear-gradient(90deg,#3BFF8C,#B6FFD0)] shadow-[0_0_16px_rgba(59,255,140,0.22)]"
          style={{ width: `${safe}%` }}
        />
      </div>
    </div>
...
export function CrmExecutiveHero({
  ...
  return (
    <CrmPanel className="overflow-hidden p-5 md:p-6">
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.9fr)]">
        <div className="relative overflow-hidden rounded-[30px] border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(59,255,140,0.16),transparent_35%),linear-gradient(180deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))] p-5 md:p-6">
          ...
          <div className="mt-5 max-w-3xl text-[32px] font-semibold leading-tight tracking-[-0.04em] text-white">
            Um pipeline comercial com leitura executiva, previsão de receita e operação clara.
          </div>

          <div className="mt-3 max-w-2xl text-sm leading-6 text-zinc-400">
            Seu CRM agora pode mostrar valor estimado, forecast ponderado e sinais reais de prioridade.
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 2xl:grid-cols-4">
            ...
            <div className="rounded-[24px] border border-white/10 bg-black/20 p-4">
              ...
              <div className="mt-2 text-sm text-zinc-400">Valor bruto em negociação</div>
            </div>

            <div className="rounded-[24px] border border-white/10 bg-black/20 p-4">
              <div className="text-[11px] uppercase tracking-[0.2em] text-zinc-500">Forecast</div>
              <div className="mt-3 text-3xl font-semibold tracking-[-0.03em] text-[#3BFF8C]">
                {formatCurrency(stats.forecastValue ?? 0)}
              </div>
              <div className="mt-2 text-sm text-zinc-400">Receita ponderada por probabilidade</div>
            </div>
            ...
            <div className="rounded-[24px] border border-white/10 bg-black/20 p-4">
              ...
              <div className="mt-2 text-sm text-zinc-400">Maturidade média do funil</div>
            </div>
          </div>
        </div>

        <div className="grid gap-5 xl:self-stretch">
          <div className="rounded-[30px] border border-white/10 bg-black/20 p-5">
            ...
            <div className="mt-4 space-y-3">
              ...
              <CrmQuickActionButton label="Relatório executivo" />
            </div>
          </div>
